...
## Modeling the Dynamic Relationship Between Compassionate and Uncompassionate Self-Responding: An Idiographic Approach
...
To address this limitation and gain deeper insights into person-specific dynamics, we conducted an *idiographic analysis* to examine the relationship between UCS and CS at the individual level [@ferrari2022embracing]. This approach allows us to explore potential heterogeneity in self-compassion processes that may be masked by nomothetic analyses, providing a more nuanced understanding of how these constructs interact within individuals over time.

## Idiographic Analysis of the Relationship Between UCS and CS

Also for this analysis, we combined the data of both studies. Each participant's data was considered separately, and we employed a multi-step procedure to calculate both the aggregate effects and the heterogeneity of these effects [e.g., @ciarrochi2024compassion; @sahdra2024testing].

### Step 1: Fitting Individual-Level Hierarchical Bayesian Models
...
### Step 2: Posterior Analysis of Gamma_CS
...
Our idiographic analysis sheds light on the complex relationship between compassionate (CS) and uncompassionate self-responding (UCS) at the momentary level. While the overall group-level results align with Neff's bipolar continuum hypothesis, significant individual variability challenges the assumption of universality in this relationship. Although the majority of participants exhibited the expected inverse association between CS and UCS, a notable proportion showed no clear relationship, and a smaller subset demonstrated positive associations—suggesting that for some individuals, CS and UCS may fluctuate together rather than oppositely, contradicting the bipolar continuum hypothesis [Ferrari et al., 2023; Ullrich et al., 2020].

Importantly, our findings revealed substantial heterogeneity in the strength of the CS-UCS relationship across individuals. This variability underscores that, for some participants, the assumed inverse relationship between CS and UCS may be weaker or even absent, suggesting that Neff's hypothesis may not apply uniformly. Such individual differences highlight the value of idiographic approaches, which capture the nuanced patterns that may be obscured in aggregate, group-level analyses.

Additionally, we found that negative affect had a consistent positive effect on UCS across participants, indicating that heightened negative emotional states tend to exacerbate uncompassionate self-responding. However, the variability in this effect across individuals further illustrates that the relationship between negative affect and UCS is not uniform. Contrary to expectations, there was no credible evidence that negative affect moderated the CS-UCS relationship, suggesting that momentary emotional states do not alter how CS influences UCS within individuals, as some previous research has suggested [Dejonckheere et al., 2021].

In summary, while the nomothetic analysis supports Neff's hypothesis at the group level, the present idiographic analysis uncovers meaningful individual differences that paint a more complex picture. For most participants, increases in CS are associated with reductions in UCS, yet for others, the relationship may be more nuanced or even contradictory. This underscores the importance of integrating both nomothetic and idiographic perspectives when examining self-compassion and related constructs. 
...
<!-- We believe that, for the purposes of testing Neff’s hypothesis, using a Bayesian multilevel model in the present study is both justified and appropriate. While more specialized idiographic approaches, such as the GIMME method [@wright2019focusing], offer specific advantages, these are not necessary for the current analysis. @wright2019focusing highlight two main strengths of GIMME over multilevel modeling: (1) it does not require assuming that all individuals follow the same functional form, and (2) it avoids assuming normally distributed random effects. However, in our case, assuming a linear relationship between UCS and CS aligns with the theoretical basis of our study and allows us to directly test the hypothesis that motivated our research. -->

<!-- Regarding the assumption of normally distributed random effects, while it is true that all statistical analyses are conditional on the model assumptions, our results are strongly supported by the good fit of the model, as evidenced by posterior predictive checks, R-hat values, and the absence of convergence issues. Thus, these assumptions do not undermine the validity of our conclusions. Moreover, multilevel modeling provides an additional advantage over purely data-driven idiographic methods by greatly attenuating the risk of overfitting. GIMME, while excellent for uncovering individual-level variation without prior assumptions, can sometimes risk overfitting when dealing with complex, noisy data, as noted by @wright2019focusing. Given our specific research question—testing the hypothesized relationship between CS and UCS—a simpler multilevel model is better suited, as it effectively bridge nomothetic and idiographic approaches, because they simultaneously estimate within-person associations among variables (e.g., daily stress→negative affect) and between-person differences in those associations [@conner2009experience]. -->
...
<!-- nomothetic (i.e., group-based)  -->

<!-- a within-person level. -->

<!-- to estimate the coefficients depicted in Fig. 1 for each individual -->
