data {
  ...
  vector[N] neg_aff_Moment;       // Covariate 1
  vector[N] context_Moment;       // Covariate 2
...
model {
  ...
  gamma_CS_raw ~ normal(-1, 1);   // Prior for CS, assuming negative relation
  ...
  // Likelihood
  UCS ~ normal(alpha + gamma_CS * CS + gamma_neg_aff * neg_aff_Moment + 
...
generated quantities {
  vector[N] y_pred;
  for (n in 1:N) {
    y_pred[n] = normal_rng(alpha + gamma_CS * CS[n] + gamma_neg_aff * neg_aff_Moment[n] + 
                           gamma_context * context_Moment[n] + phi * lag_CS_same_day[n], sigma);
  }
}
