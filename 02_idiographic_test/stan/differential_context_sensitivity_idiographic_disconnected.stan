...
parameters {
  ...
  // Random intercepts for participants, days, and measurements for CS and UCS
  vector[P] z_participant_cs; // Random intercept for participants for CS
  vector[P] z_participant_ucs; // Random intercept for participants for UCS
  ...
  real<lower=0> sigma_participant_ucs; 
...
model {
  ...
  z_participant_cs ~ normal(0, 1);
  z_participant_ucs ~ normal(0, 1);
  z_day_cs ~ normal(0, 1);
  ...
  sigma_participant_ucs ~ exponential(1);
  ...
  for (n in 1:N) {
    CS[n] ~ normal(
      alpha_cs +
      sigma_participant_cs * z_participant_cs[participant[n]] +
  ...
  for (n in 1:N) {
    UCS[n] ~ normal(
      alpha_ucs +
      sigma_participant_ucs * z_participant_ucs[participant[n]] +
