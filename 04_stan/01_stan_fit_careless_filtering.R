...
#  filtered data of Study 1, by removing the careless responding trials 
...
# Last update: Thu Jan  2 14:18:55 2025
# Status: Final
# Notes:
...
model <- cmdstan_model(
  here::here("04_stan", "study_1", "model_1_piel.stan")
)
...
if (!file.exists(
  ...
  qs::qsave(
    x = fit_mcmc,
    file = here::here("04_stan", "study_1", "filtered_cr_study1_ucs_cs.qs")
  )
} else {
  fit_mcmc <- qs::qread(
    here::here("04_stan", "study_1", "filtered_cr_study1_ucs_cs.qs")
...
posterior_summary[2:6, ]
# variable               `~mean(.x)` `~sd(.x)`    q5.5   q94.5
# 1 alpha_ucs              -0.000529   0.0403  -0.0644   0.0643
# 2 beta_cs                -0.447      0.0183  -0.477   -0.418
# 3 beta_covariates[1]      0.116      0.00589  0.107    0.125
# 4 beta_covariates[2]      0.0101     0.00550  0.00132  0.0189

# Filter the posterior draws for parameters of interest
parameters_of_interest <-
  c("alpha_ucs", "beta_cs", "beta_covariates[1]", "beta_covariates[2]")
